...
```
arXiv 论文 → HTML 提取 → 内容重组 → SpaceX 页面 → KaTeX 公式 → 暗黑配图 → OG 标签 → 产物校验 → 部署
```
...
## 步骤 8：产物校验（Fidelity Check）

> 页面做完了，必须回到原论文逐条核对。**美化不是改写，是转译。** 任何数据、公式、结论的改变都是不可接受的。

### 8.1 校验清单

逐项对照原论文，标记 PASS / FAIL：

| 校验项 | 检查什么 | 方法 |
|--------|----------|------|
| **标题** | 页面标题是否与论文一致（允许翻译差异，但核心术语不变） | 对比论文标题 |
| **摘要** | 页面摘要是否涵盖了论文摘要的所有要点，有无遗漏或添加 | 逐句对照 |
| **公式** | 每个公式的 LaTeX 是否正确（符号、上下标、变量名） | 逐公式逐字符对照 |
| **数据** | 所有实验数据（表格中的数字）是否与论文完全一致 | 逐数字对照 |
| **结论** | 页面陈述的结论是否有论文原文支撑 | 在论文中找到对应段落 |
| **超参数** | K、M、迭代次数等数值是否准确 | 对比论文实验章节 |
| **引用** | 如果提到了其他工作，引用是否准确 | 对比论文 references |
| **限制** | 页面列出的限制是否与论文声明一致，有无遗漏重要限制 | 对比论文 discussion/limitations |

### 8.2 校验流程

```
1. 打开 arXiv 原文 https://arxiv.org/html/{论文ID}v1
2. 打开页面产物（本地预览或 Preview 部署）
3. 按论文章节顺序，逐段对照页面内容
4. 对每个 FAIL 项：标记位置 → 回到原文确认 → 修正页面
5. 全部 PASS 后再进入发布流程
```
...
| 偏差类型 | 示例 | 原因 |
|----------|------|------|
| 数据四舍五入 | 论文 71.0%，页面写 71% | "好看"导致精度丢失 |
| 公式符号简化 | 论文用 `\mathcal{A}`，页面改成 `A` | 手写时偷懒 |
| 结论过度解读 | 论文说"初步结果表明"，页面写"证明" | 翻译腔/过度自信 |
| 遗漏消融实验 | 论文有 ablation study，页面没提 | 内容设计时被跳过 |
| 基线对比不完整 | 只列了对自己有利的基线 | 选择性呈现 |
